...
def list_s3_objects(s3_client, AWS_BUCKET_NAME, prefix_name):
...
def upload_files_to_s3(s3_client, AWS_BUCKET_NAME, local_data_path, prefix_name, logger):
    ...
    for root, _, files in os.walk(local_data_path):
        ...
        logger.info("Start Accessing to the folder to upload JSON files")
        ...
        if len(files) > 0:
            ...
            logger.info("Removed all JSON data in the local drive")
        else:
            print(f"No files found in directory: {root}")
            logger.error(f"No files found in directory: {root}")
